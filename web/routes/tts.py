...
def _charge(game_id: str, entry: Any, chars: int, usd: float) -> None:
    ...
    ledger = _ledger_of(entry)
    if ledger is not None:
        # Recording and persisting are one step, and the lock belongs to the
        # entry rather than to this module: the game thread, the monitor and
        # the control routes all write snapshots too, and serializing only
        # charge against charge left every other writer able to persist a
        # ledger total read before this charge landed.
        #
        # Writing it down at all matters because a `GameEntry` outlives its
        # monitor thread, which returns at the first terminal status — so for a
        # finished game, exactly the game people replay, nothing else ever
        # would, and the charge would sit in memory until a restart handed the
        # budget back.
        try:
            entry.record_cost(
                lambda: ledger.add_usd("narrator", usd, clips=1, chars=chars)
            )
